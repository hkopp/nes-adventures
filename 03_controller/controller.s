...
.proc nmi
  ...
  ; Next, we read out the controllers and handle the movement
...
ReadUp:
  ...
  beq ReadUpDone    ; branch if equal (button not pressed)
  lda $0200         ; load sprite y position
  sec
  sbc #$01          ; a--, i.e. move sprite up
  sta $0200
  ; The code below would move the second sprite but when i use it, it
  ; does not work. If I use it alone, without moving the first sprite,
  ; it works. I suspect that it is a timing issues and I cannot do
  ; enough stuff, until NMI hits again.
...
; Clear lingering interrupts since before reset: bit PPU_STATUS    ; Ack VBLANK NMI (if one was left over after reset); bit 7.
